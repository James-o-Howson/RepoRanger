...
internal sealed partial class CSharpProjectFileContentParser : IFileContentParser
{
    ...
    public async Task ParseAsync(string content, FileInfo fileInfo, Repository repository)
    {
        ...
        _logger.LogInformation("Finished Parsing CSharp Project {CsprojFilePath}. Dependencies found = {DependencyCount}", fileInfo.FullName, project.DependencyInstances.Count);
    }

    private static IEnumerable<DependencyInstance> GetDependencyContexts(string content)
    {
    ...
    private static IEnumerable<DependencyInstance> GetDependenciesFromReferenceAttribute(XContainer doc)
    {
        var dependencyViewModels = doc.Descendants()
            ...
            .Select(pr =>
            {
                ...
                return new DependencyInstance(DependencySource.Local, name, version);
            });
        
        return dependencyViewModels;
    }

    private static IEnumerable<DependencyInstance> GetDependenciesFromPackageReferenceAttribute(XContainer doc)
    {
        var dependencyViewModels = doc.Descendants()
